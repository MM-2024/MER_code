...
def extract_embedding(model_name, trans_dir, save_dir, feature_level, gpu=-1, punc_case=None, language='chinese', model_dir=None):
    ...
    if model_dir is None:
        model_dir = os.path.join(my_config.PATH_TO_PRETRAINED_MODELS, f'transformers/{model_name}')
...
if __name__ == '__main__':
    ...
    if args.punc_case is None:
        trans_dir = my_config.PATH_TO_TRANSCRIPTIONS[args.dataset]
    else:
        assert args.punc_case in ['case1', 'case2', 'case3']
        trans_dir = my_config.PATH_TO_TRANSCRIPTIONS[args.dataset][:-4] + f'-{args.punc_case}.csv'
        assert os.path.exists(trans_dir)
    save_dir = my_config.PATH_TO_FEATURES[args.dataset]
