...
if __name__ == '__main__':
    ...
    save_dir = os.path.join(my_config.PATH_TO_FEATURES[params.dataset], f'manet_{params.feature_level[:3]}')
    if not os.path.exists(save_dir): os.makedirs(save_dir)

    # load model
    model = manet(num_classes=7).cuda()
    checkpoint_file = os.path.join(my_config.PATH_TO_PRETRAINED_MODELS, 'manet/[02-08]-[21-19]-model_best-acc88.33.pth')
