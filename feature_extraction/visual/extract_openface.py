...
def extract(input_dir, process_type, save_dir, face_dir, hog_dir, pose_dir, name_npy=None):
    ...
    for i, vid in enumerate(vids, 1):
        ...
        if process_type == 'faceDir':
            exe_path = os.path.join(my_config.PATH_TO_OPENFACE_Win, 'FaceLandmarkImg.exe')
            ...
            exe_path = os.path.join(my_config.PATH_TO_OPENFACE_Win, 'FeatureExtraction.exe')
...
if __name__ == '__main__':
    ...
    input_dir = my_config.PATH_TO_RAW_FACE_Win[dataset]

    # out: feature csv dir
    save_dir = os.path.join(my_config.PATH_TO_FEATURES_Win[dataset], 'openface_all')
    hog_dir  = os.path.join(my_config.PATH_TO_FEATURES_Win[dataset], 'openface_hog')
    pose_dir = os.path.join(my_config.PATH_TO_FEATURES_Win[dataset], 'openface_pose')
    face_dir = os.path.join(my_config.PATH_TO_FEATURES_Win[dataset], 'openface_face')
