...
def resample_frames_uniform(frames, nframe=16):
    ...
    indices = np.arange(start, end, vlen / n_frms_update).astype(int).tolist() # 按照帧名数字排序
...
if __name__ == '__main__':
    ...
    print(my_config)
    face_dir = my_config.PATH_TO_RAW_FACE[params.dataset] # '/data/public_datasets/MER_2024/mer2024-dataset-process/croped_face'

    # gain save_dir
    if params.videomae_type is None:
        save_dir = os.path.join(my_config.PATH_TO_FEATURES[params.dataset], f'{model_name}-{params.feature_level[:3]}')
    else: 
        assert params.videomae_type == 'sunlicai'
        save_dir = os.path.join(my_config.PATH_TO_FEATURES[params.dataset], f'sunlicai-{model_name}-{params.feature_level[:3]}')
    ...
    if params.model_name in [CLIP_VIT_BASE, CLIP_VIT_LARGE, DATA2VEC_VISUAL, VIDEOMAE_BASE, VIDEOMAE_LARGE]: # from huggingface
        model_dir = os.path.join(my_config.PATH_TO_PRETRAINED_MODELS, f'transformers/{params.model_name}')
