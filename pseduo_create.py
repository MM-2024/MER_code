...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, epoch, optimizer=None, train=False):
    ...
    my_config.train = train # 将 train 设置为全局变量影响后面 dataloader 信息
    if train:
        model.train()
    else:
        model.eval()
...
if __name__ == '__main__':
    ...
    my_config.dataset = args.dataset
    ...
    for ii in range(len(train_loaders)):
        ...
        whole_store = []
        whole_metrics = []
        print(f'Now using {train_loader.__len__()*args.batch_size} to train and {eval_loader.__len__()*args.batch_size} samples to val')
