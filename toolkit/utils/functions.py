...
def func_split_dataset_into_two_parts(dataset, split_num):
    whole_names = []
    video_root = my_config.PATH_TO_RAW_FACE_Win[dataset]
...
def func_avi_to_mp4(video_root):
    for video_path in tqdm.tqdm(glob.glob(video_root + '/*')):
        ...
        try:
            ...
            cmd = '%s -y -i \"%s\" -loglevel quiet -y \"%s\"' %(my_config.PATH_TO_FFMPEG, video_path, save_path)
            os.system(cmd)
...
def func_video_to_face(vname):
    npy_path = os.path.join(my_config.PATH_TO_RAW_FACE[my_config.dataset], vname, vname+'.npy')
...
def func_split_audio_from_video_16k(video_root, save_root):
    ...
    for video_path in tqdm.tqdm(glob.glob(video_root+'/*')):
        ...
        cmd = "%s -loglevel quiet -y -i %s -ar 16000 -ac 1 %s" %(my_config.PATH_TO_FFMPEG, video_path, audio_path) # linux
...
def func_name_conversion(audio, suffix):
    ...
    if os.path.exists(os.path.join(my_config.PATH_TO_FEATURES['SIMS'], audio1)):
        audio = audio1
    else:
        audio = audio2
    return audio
...
def check_feature_completeness(intermerdia=None, suffix=None, target_dataset=None):
    for dataset in my_config.PATH_TO_FEATURES:
        print (f'====== {dataset} ======')
        if target_dataset != dataset: continue
        feature_root = my_config.PATH_TO_FEATURES[dataset]
...
def whether_contains_chieng():
    ...
    # for dataset in ['SIMSv2']:
        ...
        label_path = my_config.PATH_TO_TRANSCRIPTIONS[dataset]
